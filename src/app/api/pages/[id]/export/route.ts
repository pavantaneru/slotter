...
export async function GET(_req: NextRequest, {params}: {params: Promise<{id: string}>}) {
	...
	const rows = page.timeSlots.flatMap((slot) => {
		...
		if (slot.bookings.length === 0) {
			return [
				{
					slotDate: formatInTimeZone(slot.startTime, "Asia/Kolkata", "MM/dd/yyyy"),
					slotStart: formatInTimeZone(slot.startTime, "Asia/Kolkata", "HH:mm"),
					slotEnd: formatInTimeZone(slot.endTime, "Asia/Kolkata", "HH:mm"),
		...
		return slot.bookings.map((b) => {
			const responseMap = Object.fromEntries(parseGuestResponses(b.guestResponses).map((response) => [response.prompt, response.answer]));
