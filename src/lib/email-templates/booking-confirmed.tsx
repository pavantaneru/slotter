...
export function BookingConfirmedEmail({attendeeName, pageName, slotStart, slotEnd, pageSlug, appUrl, eventType, meetingLink, mapsLink}: BookingConfirmedEmailProps) {
	...
	const calendarDetails = [`Booked via Slotter: ${appUrl}/book/${pageSlug}`, venueLink ? `${venueLabel}: ${venueLink}` : null].filter(Boolean).join("\n");

	const gcalUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(pageName)}&dates=${formatInTimeZone(slotStart, "UTC", "yyyyMMdd'T'HHmmss")}/${formatInTimeZone(slotEnd, "UTC", "yyyyMMdd'T'HHmmss")}&details=${encodeURIComponent(calendarDetails)}`;

	return (
		<Html>
			<Head />
			<Preview>
				You&apos;re booked! {pageName} — {dateStr} {startStr}
			</Preview>
			<Body style={main}>
				<Container style={container}>
					...
					<Hr style={divider} />
					<Text style={footer}>This booking was made via Slotter. If you need to cancel, please contact the organizer.</Text>
				</Container>
			</Body>
		</Html>
	);
